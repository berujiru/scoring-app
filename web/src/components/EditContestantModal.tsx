...
export interface EditContestantModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
  contestantId: number
...
export default function EditContestantModal({
  isOpen,
  onClose,
  onSuccess,
  contestantId,
  initialName,
}: EditContestantModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true)
    try {
      await contestantsApi.update(contestantId, { name: name.trim() })
      onSuccess()
      onClose()
    } catch (err: any) {
      setError(err.response?.data?.error || 'Failed to update contestant')
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-bold text-gray-900">Edit Contestant</h2>
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-gray-200 justify-end">
          ...
          <button onClick={handleSubmit} disabled={isLoading} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
